		...
		<script type="text/javascript">
			function showDetail(oid){
				...
				$.post("${pageContext.request.contextPath}/adminOrder",{"method":"showDetail","oid":oid},function(d){
					...
					s+="<tr><th>商品编号</th><th>商品名称</th><th>购买数量</th><th>合计价格</th></tr>"
					
					$(d).each(function(){
						s+="<tr><td>"+this.product.pid+"</td><td>"+this.product.pname+"</td><td>"+this.count+"</td><td>"+this.subtotal+"</td></tr>";
					});
								...
								<tr
									...
									<c:forEach items="${list }" var="o" varStatus="vs">
										<tr onmouseover="this.style.backgroundColor = 'white'"
											...
											<td style="CURSOR: hand; HEIGHT: 22px" align="center"
												width="17%">
												<c:if test="${o.state == 0 }">待付款</c:if>
												<c:if test="${o.state == 1 }">
													<a id="aaa" href="${pageContext.request.contextPath }/adminOrder?method=updateState&oid=${o.oid}&action=qufahuo">去发货</a>
												</c:if>
												<c:if test="${o.state == 2 }">待收货</c:if>
												<c:if test="${o.state == 3 }">已完成</c:if>
												<c:if test="${o.state == 4 }"><a id="aaa" href="${pageContext.request.contextPath }/adminOrder?method=updateState&oid=${o.oid}&action=yituikuan">去退款</c:if>
												<c:if test="${o.state == 5 }">已退款</c:if>
											</td>
